...
// --------------- functions for FC + DOMDEG ---------------------------

vector <PDI> domdeg_vector;
void domdeg ()
{
    domdeg_vector.clear();
    for (int i = 0; i < N; i++) {
...
bool solve_using_fc_domdeg(int idx)
{
    if (idx == domdeg_vector.size()) return true;
    ...
    //     checkpoint *= 10;
    // }
    int var = domdeg_vector[idx].second;
    ...
    for (int i = 0; i < N; i++) if (domain_matrix[var][i] == 0) {
        ...
        res = solve_using_fc_domdeg(idx+1);
        if (res) return true;
        else {
