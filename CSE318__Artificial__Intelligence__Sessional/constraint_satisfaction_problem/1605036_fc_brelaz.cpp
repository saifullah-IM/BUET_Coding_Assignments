...
bool solve_using_fc_brelaz(int idx)
{
    ...
    for (int i = 0; i < N * N; i++) {
        ...
        if (sd > domain_matrix[i][N]) {
            sd = domain_matrix[i][N];
            mfd = forward_degree[i];
            var = i;
        }
        else if (sd == domain_matrix[i][N]) {
    ...
    for (int i = 0; i < N; i++) if (domain_matrix[var][i] == 0) {
        ...
        res = solve_using_fc_brelaz(idx+1);
        if (res) return true;
        else {
