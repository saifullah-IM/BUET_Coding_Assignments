...
bool solve_using_fc_domddeg(int idx)
{//cout << node_count << ' ' << idx << endl;
    ...
    for (int i = 0; i < N * N; i++) {
        ...
        double temp_ratio;
        if (forward_degree[i] != 0) temp_ratio = (domain_matrix[i][N] * 1.0) / (forward_degree[i] * 1.0);
        else temp_ratio = 1e9;
    ...
    for (int i = 0; i < N; i++) if (domain_matrix[var][i] == 0) {
        ...
        res = solve_using_fc_domddeg(idx+1);
        if (res) return true;
        else {
