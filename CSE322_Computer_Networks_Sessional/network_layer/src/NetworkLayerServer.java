...
//                System.out.println("-------------------------------" + endDevice);
    ...
    // my own
    public static boolean updateRouter(Integer routerId, boolean SF) {
        boolean result = false;
        Router router = routerMap.get(routerId);
        if (!router.getState()) return false;
        for (Integer neighbourID : router.getNeighborRouterIDs()) {
            Router neighbour = routerMap.get(neighbourID);

//            System.out.println(neighbourID);
//            System.out.println(neighbour);

            if(neighbour.getState()) {
                if (SF) result = result | router.sfupdateRoutingTable(neighbour);
                else result = result | router.updateRoutingTable(neighbour);
            }
        }
        return result;
    }

    public static synchronized void DVR(Integer startingRouterId) {
        /**
         * pseudocode
         */
        ...
        */
        while(true) {
            boolean result = updateRouter(startingRouterId,true);
            for (Router router : routers) {
                if (router.getRouterId().equals(startingRouterId)) continue;
                result = result | updateRouter(router.getRouterId(), true);
            }
            if (!result) break;
    ...
    public static synchronized void simpleDVR(Integer startingRouterId) {
//        System.out.println("Start at " + startingRouterId);
        while(true) {
            boolean result = updateRouter(startingRouterId,false);
            for (Router router : routers) {
                if (router.getRouterId().equals(startingRouterId)) continue;
                result = result | updateRouter(router.getRouterId(), false);
            }
            if (!result) break;
        }
    }
    ...
    public static EndDevice getClientDeviceSetup() {
        ...
        for (Map.Entry<IPAddress, Integer> entry : clientInterfaces.entrySet()) {
            ...
            if(i == r) {
                ...
                value++;
                deviceID = endDevices.size();
                clientInterfaces.put(key, value);
                deviceIDtoRouterID.put(deviceID, interfacetoRouterID.get(key));
                break;
            }
            i++;
        }
    ...
    public static void readTopology() {
        ...
        for (int j = 1; j <= 100; j++) tempIDs.add(j);
        int maxRouter = 100;
        ...
        try {
            ...
            while(inputFile.hasNext()) {
                ...
                if (id > maxRouter) {
                    for (int j = maxRouter + 1; j <= id; j++) tempIDs.add(j);
                    maxRouter = id;
                }
                routerId = tempIDs.get(id - 1);
                ...
                for(int i = 0; i < count; i++) {
                    ...
                    Integer neighbourID = tempIDs.get(id - 1);
                    neighborRouters.add(neighbourID);
                }
